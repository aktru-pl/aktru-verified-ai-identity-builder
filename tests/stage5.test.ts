...
const registry:RegistryResult={fileName:'synthetic-krs.json',valid:true,sha256:'ABC',message:'ok',fields:[{field:'name',value:'EXAMPLE SP Z OO',path:'$.odpis.dane.nazwa',sourceClass:'supported by supplied registry document'},{field:'nip',value:'1234567890',path:'$.odpis.dane.nip',sourceClass:'supported by supplied registry document'}]};
...
describe('stage 5 product behavior',()=>{
 ...
 it('detects mismatch',()=>expect(compareValue('name','Hospitality Example','AKTRU').status).toBe('mismatch'));
 it('detects a missing website field',()=>expect(compareValue('regon',null,'123').status).toBe('missing on website'));
 it('does not match synthetic AKTRU registry to a hospitality example',()=>expect(compareValue('name','Hospitality Example Hotel','AKTRU sp. z o.o.').status).toBe('mismatch'));
